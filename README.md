Based on the provided screenshots, here is the reconstructed **README.md** file for the Campaign Management System.

---

# Campaign Management System — Agentic AI (Email)
...
| **Email-only** | Single-channel focus — no SMS, push, or social. |
...
| **Person-level Tracking** | Live pipeline visualization — see which agents are running for which campaign/branch/variant. |
| **Unsubscribe Management** | One-click unsubscribe with HTML landing page; suppressed contacts never receive future emails. |
...
| --- | --- |
| **Architecture & Design** | `docs/campaign_mgmt_agentic_ai.md` |
| **Deployment Guide** | `docs/campaign_mgmt_deployment_guide.md` |
| **Real Email Tracking** | `docs/real_email_tracking.md` |
| **Project Overview** | `docs/campaign_mgmt_overview.txt` |
...
│   ├── executor.py         # Delivery with dedup, A/B split, unsubscribe check
...
├── docs/                   # Architecture, deployment, tracking guides
└── deploy/
...
9. **Execute**: Emails sent per branch/variant at scheduled time.
...
| **EmailEvent** | Campaign-branch-level engagement (legacy, no recipient identity). |
...
* **UI**: [http://localhost:8501](https://www.google.com/search?q=http://localhost:8501)
